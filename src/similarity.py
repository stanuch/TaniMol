...
def save_similarity_matrix(similarity_matrix, path):
    if np.isnan(similarity_matrix).any():
        raise ValueError("Warning: The similarity matrix contains NaN (Not a Number) values. Matrix is corrupted.")
    
    if similarity_matrix.shape[0] != similarity_matrix.shape[1]:
        raise ValueError(f"Warning: Matrix must be a square (NxN), got {similarity_matrix.shape} instead.")

    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
    np.save(path, similarity_matrix)
    print(f"Saved {similarity_matrix.shape[0]}x{similarity_matrix.shape[1]} similarity matrix to {path}")
